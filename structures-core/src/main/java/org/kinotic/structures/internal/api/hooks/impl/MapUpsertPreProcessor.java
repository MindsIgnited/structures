...
public class MapUpsertPreProcessor implements UpsertPreProcessor<Map<Object, Object>, List<Map<Object, Object>>, Map<Object, Object>> {
    ...
    private Pair<String, DecoratorLogic> tenantIdFieldPreProcessor = null;
    ...
    public MapUpsertPreProcessor(Structure structure,
        ...
        // this is a temporary solution since we only have an id, tenant id and version field preprocessor now.
        for(Map.Entry<String, DecoratorLogic> entry : fieldPreProcessors.entrySet()) {
            ...
            if(decorator instanceof IdDecorator
                ...
                tenantIdFieldPreProcessor = Pair.of(entry.getKey(), entry.getValue());
    ...
    private EntityHolder<Map<Object, Object>> preProcessData(Map<Object, Object> entity, EntityContext context) {
        ...
        String id = processField(idFieldPreProcessor, entity, context);
        ...
        if(structure.getMultiTenancyType() == MultiTenancyType.SHARED){
            ...
            if(structure.isMultiTenantSelectionEnabled()){
                ...
                String tenantIdFieldName = tenantIdFieldPreProcessor.getLeft();
                tenantId = processField(tenantIdFieldPreProcessor, entity, context);
                if(tenantId == null || tenantId.isBlank()){
                    throw new IllegalArgumentException("Could not find TenantId for Entity");
                }

                entity.put(tenantIdFieldName, tenantId);
    ...
    private String processField(Pair<String, DecoratorLogic> fieldPreProcessor,
                                Map<Object, Object> entity,
                                EntityContext context) {
        String fieldName = fieldPreProcessor.getLeft();
        DecoratorLogic decoratorLogic = fieldPreProcessor.getRight();
        UpsertFieldPreProcessor<C3Decorator, Object, Object> preProcessor = decoratorLogic.getProcessor();

        // the Id and TenantId decorators both process a String field, so we can cast it
        return (String) preProcessor.process(structure,
                                             fieldName,
                                             decoratorLogic.getDecorator(),
